...
- [ ] **多天调度模型（Step 1.6，核心待做，用户主诉求）**：现在 `schedule_node` 把全量 task_store 拆解**全塞进今天**，不看日期也不跨天分配。目标：新增**推理模型驱动的多天规划层**——输入活跃任务（deadline/工时/优先级/负荷/source_excerpt）+ 规划视野（今天→最晚 deadline，设上限）+ **每天空闲容量**（前端上传未来 N 天本地日历算出，已定用这个方案），输出把每个任务工时**切块摊到多天**（尊重 deadline、每天容量、临期留缓冲）。未来到期 + 无日期的都要纳入、可提前做。每天日程 = 当天分到的块，交现有精力感知调度器摆位。生成/写入/勾完成后重算。
