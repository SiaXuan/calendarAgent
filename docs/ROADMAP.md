...
| **D** | 用户自带 MCP 扩展接口 | 🔨 | **脚手架已就位（2026-08-25）**：`graphs/user_mcp.py`（langchain-mcp-adapters 把 MCP 工具接进 `create_react_agent` 工具带，按 server allowlist 收窄、启动加载一次、优雅降级）+ `UserPreferences.custom_mcp_servers`（PATCH 可配 + 热重载）+ `requirements` 加 `langchain-mcp-adapters`。**待做**：接真 server（用户选**先 Outlook**，走 MS Graph MCP + Entra OAuth）、`add_to_schedule` 桥（agent 抽出邀请 → 走 EventKit 写 Apple 日历，因为日历写在 Swift 端不在 MCP）、工具收窄 UX。`chronotype` 仍只是 `health_agent` 中间值、非可配字段。明确不做：MEQ 问卷 / role 类目 / 预定义 task_lexicon |
